...
# python -m table2text_try.run_language_modeling_final \
python -m table2text.run_language_modeling \
  ...
  --learning_rate ${learning_rate} --lr_decay "no" --num_train_epochs ${num_train_epochs} --per_device_train_batch_size 200 --gradient_accumulation_steps 6 \
  --non_private ${non_private} \
  --clipping_mode "${clipping_mode}" \
  --cache_dir ${cache_dir} --gpu_id ${gpu_id} --noise_type Gaussian
