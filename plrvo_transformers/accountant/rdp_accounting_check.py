...
def Mu(
    ...
    if "Gamma" in params["distributions"]:
        try:
            ...
            if 1-params['a_G']*t*params['G_theta'] == 0:
                return np.inf
            pass
    ...
    if distributions_used != params["distributions"]:
        # print(distributions_used)
        # print(params["distributions"])
        raise ValueError
    else:
        return MGF
...
def compute_binomial_log(n, k):
    """ log(binomial(n, k))"""
    return math.lgamma(n + 1) - math.lgamma(k + 1) - math.lgamma(n - k + 1)



def _compute_G(S, C, eta):
    G_k = S['G_k']
    G_theta = S['G_theta']
    ...
    if 1 - t1 * S['G_theta'] < 0:
        ...
        G = 0.5 * (Mu(t1, S) + Mu(t2, S)) + (1/(2*(2*eta-1))) * (Mu(t1, S) - Mu(t2, S))
...
def _compute_plrvo_for_int_alpha3(zeta, S, alpha, C):
    log_terms = []
    for eta in range(alpha + 2):
        # print(alpha, eta)
        log_binom = compute_binomial_log(alpha + 1, eta)
        log_term = (
            log_binom
            + (alpha + 1 - eta) * math.log(1 - zeta)
            + eta * math.log(zeta)
            + _compute_G(S, C, eta)
        )
...
# PLRV-O: TODO Eq. 6 for each \alpha_{M_q}(\lambda)
def _compute_plrvo_for_int_alpha3_again(q, params, alpha, L2_sensitivity) -> float:
    summation = 0
    for eta in range(alpha + 2):
        # TODO avoid overflow
        log_factorials = np.zeros(alpha + 1)
        ...
        weight = ((1 - q) ** (alpha + 1 - eta)) * (q ** eta)
        
        t1 = L2_sensitivity * (eta - 1)
        t2 = - eta * L2_sensitivity
        if t1<=1/params["G_theta"]:
            G = 0.5 * () + (1/(2*(2*eta-1))) * ()
            summation = summation + binom_coeff * weight * Mu(t=t, params=params)
    ...
    if math.log(summation) < 0:
        # TODO
        print("math.log(summation) < 0: when ", alpha, eta)
        return np.inf
    else:
        return math.log(summation)


def _compute_plrvo_for_int_alpha3_backup(q, params, alpha, L2_sensitivity) -> float:
    C = L2_sensitivity
    clip = L2_sensitivity
    theta = params["G_theta"]
    k = params["G_k"]
