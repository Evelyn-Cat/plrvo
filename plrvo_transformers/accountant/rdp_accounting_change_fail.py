...
#             if debug_mode:
#                 check_M_G = 1-params['a_G']*t*params['G_theta']
#                 print(f"t: {t}; G_theta: {params['G_theta']}")
#                 print(f"1-params['a_G']*t*params['G_theta']: {check_M_G}")
#             MGF = MGF * M_G
...
#         if debug_mode:
#             print("Now returning 0 for Mu ...")
#             print(f"distributions_used: {distributions_used}")
#             print(f"params[\"distributions\"]: {params["distributions"]}")
#         return 0
#     else:
#         return MGF

# ## PLRV-O: A_\alpha for PLRV-O
# def _compute_log_a_p(q: float, S: Dict, C: float, alpha: float) -> float:
#     r"""Computes :math:`log(A_\alpha)` for any positive finite ``alpha``.
...
#         S: The noise parameters of the PLRV-O noise.
#         C: Clipping threshold in PLRV-O DPSGD.
#         alpha: The order at which RDP is computed.
...
# ## PLRV-O: RDP of PLRV-O in one step
# def _compute_rdp_p(q: float, S: Dict, C: float, alpha: float) -> float:
...
#     return _compute_log_a_p(q, S, C, alpha)


# ## PLRV-O: RDP of PLRV-O
# def compute_rdp_p(
#     q: float, S: Dict, steps: int, orders: Union[Sequence[float], float], C: float,
# ) -> Union[List[float], float]:
#     r"""Computes Renyi Differential Privacy (RDP) guarantees of the
...
#     """
#     if isinstance(orders, float):
#         rdp = _compute_rdp_p(q, S, C, orders)
#     else:
#         rdp = np.array([_compute_rdp_p(q, S, C, order) for order in orders])
