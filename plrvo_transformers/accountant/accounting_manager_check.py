...
from . import rdp_accounting_check as rdp_accounting
...
class RDPManager(AccountingManager):
    ...
    def _compute_epsilon_from_distribution2(self, params, sample_rate, target_delta, steps):
        return self.compute_epsilon_p2(params, sample_rate, target_delta, steps)["epsilon"]
    
    # PLRV-O: accounting new
    def compute_epsilon_p2(self, params, sample_rate, target_delta, steps) -> Dict:
        """convert to (ε, δ)-DP based on the result by Canonne, Kamath, Steinke."""
        rdp = rdp_accounting.compute_rdp_p2(params=params, q=sample_rate, steps=steps, orders=self._alphas, L2_sensitivity=self._L2_sensitivity)
        epsilon, alpha_rdp = rdp_accounting.get_privacy_spent(orders=self._alphas, rdp=rdp, delta=target_delta)
        print("opt_rdp_alpha: ", alpha_rdp)
        return dict(epsilon=epsilon, alpha_rdp=alpha_rdp)

    # PLRV-O: accounting new
    def _compute_epsilon_from_distribution3(self, params, sample_rate, target_delta, steps):
        return self.compute_epsilon_p3(params, sample_rate, target_delta, steps)["epsilon"]
    
    # PLRV-O: accounting new
    def compute_epsilon_p3(self, params, sample_rate, target_delta, steps) -> Dict:
        """convert to (ε, δ)-DP based on the result by Canonne, Kamath, Steinke."""
        rdp = rdp_accounting.compute_rdp_p3(params=params, q=sample_rate, steps=steps, orders=self._alphas, L2_sensitivity=self._L2_sensitivity)
        print(rdp)
        epsilon, alpha_rdp = rdp_accounting.get_privacy_spent(orders=self._alphas, rdp=rdp, delta=target_delta)
        print("opt_rdp_alpha: ", alpha_rdp)
        return dict(epsilon=epsilon, alpha_rdp=alpha_rdp)
